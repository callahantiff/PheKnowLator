...
def main():
    ...
    print('INPUT DOCUMENT BUILDER\n\nThis program will help you generate the input documentation needed to run '
          ...
          '\n\t(2) prefixes: A ";"-separated string where the first item is the final prefix for the subject node and '
          'the second is the final prefix for the object node. . All prefixes should be the '
          'preferred prefix from the BioRegistry\n\t\t(https://bioregistry.io/registry/);\n\t(3) relation: An OBO '
          'Foundry ontology CURIE (e.g., RO_0000056)\n\t(4) delimiter: A character used to split rows from an input '
          'data source into columns (e.g., "t" for tab-delimited data or "," for comma-delimited data);\n\t(5) '
          'column_indexes: Two-column indexes separated by ";" (e.g., "0;4" for the first and third columns in the '
          'input data source);\n\t(5) IdentifierMaps: A string of mapping information for each node in an edge. For '
          'example, the string "2:mapping_file_1.txt;4:mapping_file_2.txt" means that the first node require\n\t\tdata '
          'contained in the 2nd column of the "mapping_file_1.txt" and the second node requires data from the 4th '
          'column in the "mapping_file_2.txt" file;\n\t(6) evidence_criteria: Evidence criteria that can be used to '
          'filter an input data source (e.g., scores above a certain cut-off). An evidence set is composed of 3 pieces '
          'of ";"\n\t\t-separated information. Multiple filtering sets can be passed, where each set is separated by '
          '"::". Consider the following example: "4;!=;IEA::8;<;0.0001"):\n\t\t\t1. The index of the column to apply '
          'the evidence criteria to (e.g., "4" and "8" in the example above)\n\t\t\t2. The operator (i.e., "==", '
          '"!=", "<", ">", "<=", ">=", "in", ".startswith()", ".endswith()") to use when filtering (e.g., "!=" and '
          '"<" in the example above).\n\t\t\t3.The value (i.e., "int", "float", "str", "list") to filter on (e.g., '
          '"IEA" and "0.0001" in the example above);\n\t(7) filtering_criteria: Criteria that can be used to filter '
          'an input data source (e.g., human proteins). An evidence set is composed of 3 pieces of ";"-separated '
          'information. \n\t\tMultiple filtering sets can be passed as demonstrated by the example above, where each '
          'set is separated by "::". Consider the following example: "5;==;P::7;==;9606"):\n\t\t\t1. The index of the '
          ...
          'is shown below:\n\t\tchemical-gene|;MESH;|RO_0002434|#|t|1;4|0:./resources/data_maps/'
