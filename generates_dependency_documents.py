...
def main():
    ...
    print('INPUT DOCUMENT BUILDER\n\nThis program will help you generate the input documentation needed to run '
          ...
          'file;\n\t(8) EvidenceCriteria: Evidence criteria that can be used to filter an input data source (e.g., '
          ...
          '"0.0001" in the example above);\n\t(9) Filtering criteria that can be used to filter an input data source '
          '(e.g., human proteins). An evidence set is composed of 3 pieces of ";"-separated information.\n\t\tMultiple '
          'filtering sets can be passed as demonstrated by the example above, where each set is separated by "::". '
          'Consider the following example: "5;==;P::7;==;9606"):\n\t\t\t1. The index of the column to apply the '
          'evidence criteria to (e.g., "5" and "7" in the example above)\n\t\t\t2. The operator (i.e., "==", "!=", '
